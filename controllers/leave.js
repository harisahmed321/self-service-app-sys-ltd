...
addRequest = (req, res, next) => {
  ...
  const updateLeaveQuota = (leave) => {
    QuotaModel.findOne({ userId: mongoose.Types.ObjectId(userId) })
      .then((quota) => {
        QuotaModel.findOneAndUpdate(
          ...
          (err) => {
            if (err) next(error);
              ...
              res
                .status(200)
                .send(success(leave, 'Leave request added successfully'));
          }
        );
      })
      .catch((error) => {
...
leavesByTeam = (req, res, next) => {
  ...
  TeamManagementModel.findOne({ managerId })
    .then((result) => {
      fetchLeaveRequest(result.empIds);
    })
